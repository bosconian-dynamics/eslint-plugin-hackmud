...
function post(messages, filename) {
  messages = messages[0].map(function (message) {
    ...
    message.message = message.message.replace(/$(\w?S_)/g, '#$1s.').replace(_config.DATABASE_PREFIX, '#db.').replace(_config.PREPROC_D, '#D').replace(_config.PREPROC_G, '#G').replace(_config.PREPROC_FMCL, '#FMCL');
